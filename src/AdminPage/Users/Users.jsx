...
export default function Users() {
  ...
  let initialState = { fName: user.fName, lName: user.lName, email: user.email };
  ...
  useEffect(() => {
    initialState = { fName: user.fName, lName: user.lName, email: user.email };
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.values(formError).every((value) => !value.trim())) {
      await axios.put('http://localhost:2500/user/' + user._id, formState,
        { headers: { Authorization: `Bearer ${localStorage.token}` } })
        .then((res) => { setUser(res.data), setSuccessPopUp(true) })
  ...
  return (
    <form className={styles.users} onSubmit={handleSubmit}>
      {successPopUp &&
      ...
      <ul className={styles.fields}>
        ...
        <li className={styles.joinDate}>
          <b>תאריך הצטרפות: </b> {formatDate(user.createTime).date}, {formatDate(user.createTime).time}
        </li>
        {isChange &&
